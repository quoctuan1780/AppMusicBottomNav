...
public class Online extends Fragment  {
    private View view;
    private TextView tv_xemthem_album, tv_xemthem_casi, tv_xemthem_playlist;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        tv_xemthem_playlist = (TextView) view.findViewById(R.id.tv_playlist_xemthem);
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if(getContext() != null && playlistArrayList != null && playlistArrayList.size() != 0){
            ArrayList<Playlist> playlists = new ArrayList<>();
            if(playlistArrayList.size() > 3) {
                playlists.add(playlistArrayList.get(0));
                playlists.add(playlistArrayList.get(1));
                playlists.add(playlistArrayList.get(2));
                playlistOnlineAdapter = new PlaylistOnlineAdapter(getContext(), playlists, getParentFragment(), session);
            }else
                playlistOnlineAdapter = new PlaylistOnlineAdapter(getContext(), playlistArrayList, getParentFragment(), session);
        ...
        if(getContext() != null && albumarraylist != null && casiArrayList != null && baihatArrayList != null
            && !albumarraylist.isEmpty() && !casiArrayList.isEmpty() && !baihatArrayList.isEmpty() && !slideArrayList.isEmpty()){
            ...
            casiOnlineRandomAdapter = new CasiOnlineRandomAdapter(getContext(), casiArrayList, getParentFragment());
            ...
            if(!session.getToken().equals("")) {
                ...
                Toast.makeText(getContext(), "Bạn chưa đăng nhập, không xem thêm được Playlist", Toast.LENGTH_LONG).show();
            }
            xemThemAlbum();
        }else {
            try {
    ...
    private void xemThemPlayList(){
        tv_xemthem_playlist.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                NavHostFragment.findNavController(Online.this)
                        .navigate(R.id.action_item_online_to_danhSachPlayListOnline);
            }
        });
    }

    private void xemThemCaSi(){
    ...
    public void laycasigoiy(){
        try{
            DataService dataService = APIService.getService();
            Call<List<Casi>> listcasi = dataService.LaydulietcasiRandom();
    ...
    private void layPlaylist(){
        try{
            ...
            if(!session.getToken().equals("")) {
                ...
                Toast.makeText(getContext(), "Bạn chưa đăng nhập, không lấy được Playlist", Toast.LENGTH_LONG).show();
            }
        }catch(Exception e){
